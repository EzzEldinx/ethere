/* global jQuery, HALL_STORAGE */

jQuery( document ).on( 'action.ready_hall', function() {
	...
	$document.on( 'action.resize_hall', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.hall_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		hall_set_cookie( 'hall_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// Add buttons inc/dec
	function hall_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						hall_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function hall_woocomerce_inc_dec_click(button) {
			...
			.on( 'click', function(e) {
				...
				jQuery.get(	link ).done(
					function( response ) {
						...
						hall_import_inline_styles( response );
						// Get tags 'link' from response and add its to the 'head'
						hall_import_tags_link( response );
						...
						if ( posts_container.length > 0 ) {
							...
							HALL_STORAGE['init_all_mediaelements'] = true;
							$document
	...
	$document.on( 'action.scroll_hall', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < hall_window_scroll_top() + hall_window_height() * 1.5 ) {
	...
	//-------------------------------------------------------------------------
	if ( ! HALL_STORAGE['select_container_disabled'] ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		hall_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					hall_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					hall_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		hall_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			hall_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function hall_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = hall_window_width() + ( hall_window_height() < $document.height() || hall_window_scroll_top() > 0 ? 16 : 0 );
				}

				var tbl = jQuery( 'table.cart' );   // table_cart;

				if ( w < HALL_STORAGE['mobile_layout_width'] ) {
				...
				if ( w >= HALL_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
